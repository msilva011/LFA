class MTU
  ...
  def processar(entrada)
    ...
    while true
      # Verificação e depuração do estado atual e símbolo na fita
      puts "Estado atual: #{@estado}, Símbolo na fita: #{@fita[@cursor]}, Cursor: #{@cursor}"
      ...
      in [:q21, 'b'] # recomeça a leitura
        ...
        if transicoes.key?(leitura)
          ...
          puts "Transição não encontrada para (#{estado_mt}, #{simbolo_leitura})"
          return false
        end
